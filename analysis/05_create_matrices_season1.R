...
net_core <- net_core %>% 
  mutate(season = case_when(month %in% c("Apr", "Sept") ~ "tail",
                            month %in% c("Jun", "Jul") ~ "peak",
                            TRUE ~ "shoulder"), .after = day)
...
save(fish.list.season, file = here("data", "fish.list.season1.Rdata")) #last saved 04/30/25
