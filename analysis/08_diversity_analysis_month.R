...
save(mFD_results.month, file = here("data","mFD_results.month.Rdata")) #last saved 4/26/25
...
plot_prep <- mFD_results %>%
  ...
  ungroup() %>% 
  pivot_longer(!c(shoreline, site, ipa, ipa2, month, region, veg, id), 
...
mFD_averages <- mFD_results %>% 
  pivot_longer(!c(shoreline, site, ipa, month, region, veg), names_to = "metric", values_to = "value") %>% 
...
#### figure S2 ####
mFD_results %>% 
  pivot_longer(!c(site, ipa, month, shoreline, region, veg), names_to = "metric", values_to = "value") %>% 
